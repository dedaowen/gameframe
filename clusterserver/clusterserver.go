...
func (this *ClusterServer) StartClusterServer() {
	...
	if len(serverconf.Http) > 0 {
		...
		logger.Info(fmt.Sprintf("https://%s:%d start", serverconf.Host, int(serverconf.Https[0].(float64))))
	}
	//tcp server
	if serverconf.NetPort > 0 {
		utils.GlobalObject.TcpPort = serverconf.NetPort
		if utils.GlobalObject.IPVersion == "ws" {
			if serverconf.WsPath != "" {
				this.NetServer = fserver.NewWsServer("cluster_net_wsserver", serverconf.WsPath, serverconf.NetPort,
					utils.GlobalObject.MaxConn, utils.GlobalObject.Protoc)
			} else {
				this.NetServer = fserver.NewWsServer("cluster_net_wsserver", utils.GlobalObject.WsPath, serverconf.NetPort,
					utils.GlobalObject.MaxConn, utils.GlobalObject.Protoc)
			}
		} else {
			if serverconf.Host != "" {
				this.NetServer = fserver.NewTcpServer("cluster_net_server", "tcp4", serverconf.Host, serverconf.NetPort,
					utils.GlobalObject.MaxConn, utils.GlobalObject.Protoc)
			} else {
				this.NetServer = fserver.NewTcpServer("cluster_net_server", "tcp4", utils.GlobalObject.Host, serverconf.NetPort,
					utils.GlobalObject.MaxConn, utils.GlobalObject.Protoc)
			}
		}
	...
	if serverconf.RootPort > 0 {
		if serverconf.Host != "" {
			this.RootServer = fserver.NewTcpServer("cluster_root_server", "tcp4", serverconf.Host, serverconf.RootPort,
				utils.GlobalObject.IntraMaxConn, utils.GlobalObject.RpcSProtoc)
		} else {
			this.RootServer = fserver.NewTcpServer("cluster_root_server", "tcp4", utils.GlobalObject.Host, serverconf.RootPort,
				utils.GlobalObject.IntraMaxConn, utils.GlobalObject.RpcSProtoc)
		}
	...
	logger.Info("cluster start success.")
	// close
	this.WaitSignal()
